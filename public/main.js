...
// Controls
const textInput = $("#wmText");
const fontSizeInput = $("#wmFontSize");
const fontFamilyInput = $("#wmFontFamily");
const colorInput = $("#wmColor");
const rotateInput = $("#wmRotate");
const repeatInput = $("#wmRepeat");
const gapXInput = $("#wmGapX");
const gapYInput = $("#wmGapY");
const paddingInput = $("#wmPadding");
const layoutAngleInput = $("#wmLayoutAngle");
const watermarkOpacityInput = $("#wmOpacity");
const wmImageInput = $("#wmImageInput");
const wmImageScaleInput = $("#wmImageScale");
const wmImageOpacityInput = $("#wmImageOpacity");
// Anti-tamper controls
const hardenInput = $("#wmHarden");
const blendSelect = $("#wmBlend");
const jitterInput = $("#wmJitter");
const textureInput = $("#wmTexture");
const textureAlphaInput = $("#wmTextureAlpha");
const invisibleInput = $("#wmInvisible");
const hiddenTextInput = $("#wmHiddenText");
const exportSelect = $("#wmExport");
const jpegQInput = $("#wmJpegQ");
let baseImage = null;
let currentOptions = {
    ...
    blendMode: blendSelect?.value || 'source-over',
    ...
    exportFormat: exportSelect?.value || 'png',
...
function applyWatermark(base, options, targetCanvas) {
    ...
    const textW = Math.ceil(Math.max(textMetrics.width, options.image ? (options.image.width * options.imageScale) / 100 : 0));
    const textH = Math.ceil(Math.max(options.fontSize, options.image ? (options.image.height * options.imageScale) / 100 : 0));
...
function refreshOptionsFromInputs() {
    currentOptions = {
        text: textInput.value,
...
function embedInvisibleWatermark(dest, text) {
    if (!text)
        return;
...
function downloadWithOptions(viewCanvas, options) {
